...
- [ ] 1.2 新增 `AgentRuntime` 适配层测试，覆盖 `FakeAgentRuntime` 默认测试路径和真实 runtime 缺失返回 `AGENT_RUNTIME_UNAVAILABLE`。
- [ ] 1.3 新增 agent 状态机测试，覆盖完整合法路径、错误 stage 调用 tool 返回 `AGENT_INVALID_TRANSITION`、非法调用不执行副作用且不重试。
...
- [ ] 1.10 新增重复来源测试，覆盖 SQLite `status = "processed"` 命中时返回 `skipped_existing`、写 state/trace、不执行后续副作用 tools。
- [ ] 1.11 新增有限重试测试，覆盖网络/API/下载错误最多重试一次，schema、配置、Whisper、写入和非法转换错误不重试。
...
- [ ] 2.1 在 `pyproject.toml` 增加 `agent` optional extra，放入真实 Deep Agents runtime 依赖，并保持默认安装不要求该 extra。
...
- [ ] 2.4 新增 `AgentRuntime` 接口、`DeepAgentsRuntime` 生产适配器和 `FakeAgentRuntime` 测试替身。
...
- [ ] 3.2 实现 Python 状态机 guard，按 tool 和当前 stage 校验合法转换，非法转换不执行副作用。
...
- [ ] 3.7 实现重试策略组件，网络/API/下载类错误每个 tool 最多重试一次，其余错误不重试。

## 4. 受控 Python Tools

- [ ] 4.1 实现 `route_url` tool，复用既有 URL 规范化和内容类型识别逻辑，输出 `normalized_url`、`source_id` 和 `content_type`。
- [ ] 4.2 实现 `prepare_source_workspace` tool，初始化素材目录、SQLite 边界、agent 目录、state/trace，并处理 processed 重复来源跳过。
