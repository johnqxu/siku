...
当前唯一公开导入入口是 `km agent-ingest`，由项目内部 Deep Agents 编排受控 Python tools，提供 agent 编排、state/trace 可观测性和自动产物复用。输入和输出都使用 JSON，方便 Hermes 或其他上层调度器做重试、错误处理和状态跟踪。
...
- `agent` extra 提供公开导入入口需要的 Deep Agents runtime，写入 `<asset_dir>/agent/state.json` 和 `<asset_dir>/agent/trace.jsonl` 便于观察 agent 编排。
...
- `[llm.tasks] domain_classification` 指定领域分类模型，由 `km agent-ingest` 内部受控工具使用。
- `[llm.tasks] summary_generation` 指定中文总结主模型，由 `km agent-ingest` 内部受控工具使用。
- `[llm.tasks] agent_orchestration` 指定 Deep Agents 编排模型，是公开导入入口 `km agent-ingest` 的必需配置；缺失时会返回 `CONFIG_INVALID`。
...
`km agent-ingest` 使用 JSON stdin 契约：
